...
# 시간대별 평균 전력 소비량 및 평균 온도 계산-----------------------------------------
avg_power_temp_hour = df.groupby('hour').agg({
    'target': 'mean',
    'temp': 'mean'
})
...
ax1.set_ylabel('Average Power Consumption (kWh)', color='tab:blue')
ax1.plot(avg_power_temp_hour.index, avg_power_temp_hour['target'], marker='o', linestyle='-', color='tab:blue', label='Power Consumption')
ax1.tick_params(axis='y', labelcolor='tab:blue')

# 두 번째 y축 (온도)
ax2 = ax1.twinx()  # 동일한 x축을 공유하는 두 번째 y축 생성
ax2.set_ylabel('Average Temperature (°C)', color='tab:red')  
ax2.plot(avg_power_temp_hour.index, avg_power_temp_hour['temp'], marker='s', linestyle='--', color='tab:red', label='Temperature')
ax2.tick_params(axis='y', labelcolor='tab:red')

# 타이틀과 레이아웃 설정
plt.title('Average Power Consumption and Temperature by Hour')
fig.tight_layout()  # 레이아웃 조정
plt.show()

# 시간대별 평균 전력 소비량 및 평균 습도 계산------------------------------------------
avg_power_temp_hour = df.groupby('hour').agg({
    'target': 'mean',
    'humid': 'mean'
})
...
ax2 = ax1.twinx()  # 동일한 x축을 공유하는 두 번째 y축 생성
ax2.set_ylabel('Average Temperature (°C)', color='tab:red')  
ax2.plot(avg_power_temp_hour.index, avg_power_temp_hour['humid'], marker='s', linestyle='--', color='tab:red', label='Temperature')
ax2.tick_params(axis='y', labelcolor='tab:red')
