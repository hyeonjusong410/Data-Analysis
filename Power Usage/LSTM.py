...
for idx, (num, group) in enumerate(df.groupby('num')):
    ...
    class LSTMModel(nn.Module):
        def __init__(self, input_size, hidden_size, num_layers, output_size):
            ...
            self.lstm = nn.LSTM(input_size, hidden_size, num_layers, batch_first=True)  # 단방향 LSTM
            self.fc = nn.Linear(hidden_size, output_size)  # hidden_size만 사용

        def forward(self, x):
            # hidden state 및 cell state 초기화
